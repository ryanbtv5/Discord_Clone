...
import { Hash, Phone, Video, Users, Search, Smile, Plus, ImageIcon } from "lucide-react";
import { format, isToday, isYesterday } from "date-fns";
import type { Channel, Server, MessageWithUser } from "@shared/schema";
...
export default function ChatArea({ channel, server, onUserClick }: ChatAreaProps) {
  ...
  useEffect(() => {
    const eventSource = new EventSource(`/api/channels/${channel.id}/events`);
    
    eventSource.onmessage = (event) => {
      const data = JSON.parse(event.data);
      if (data.type === 'message') {
        queryClient.setQueryData(["/api/channels", channel.id, "messages"], (oldMessages: MessageWithUser[] | undefined) => {
    ...
    <div className="flex-1 flex flex-col bg-discord-dark">
      ...
      <div className="h-16 border-b border-discord-darkest flex items-center justify-between px-4">
        <div className="flex items-center">
          ...
          {channel.description && (
            <div className="ml-2 text-discord-text-muted text-sm">{channel.description}</div>
          )}
        </div>
